...
If using the zipped project, unzip all files into one directory and open that directory in a terminal.
...
For production or public use, do not hard-code API keys. Store the OpenAI API key in an environment variable:

```bash
export OPENAI_API_KEY="your-api-key-here"
```

The current codebase should be updated to read this value from the environment before deployment.
...
|---|---|---|
| `/` | GET | Homepage |
| `/upload` | GET | Scan upload page |
| `/prediction` | GET/POST | Central image/report processing endpoint |
| `/summarizer` | GET | Report analyzer upload page |
| `/chatbot` | POST | Chatbot API endpoint |
| `/download_report` | POST | Generate and download PDF summary |
...
Before deploying this project publicly, the following changes are strongly recommended:

- Move API keys and Flask secrets into environment variables.
- Disable Flask debug mode in production.
- Use `werkzeug.utils.secure_filename` for uploaded files.
- Validate file MIME types, not just extensions.
- Add file size limits.
- Store uploads outside public static paths where possible.
- Sanitize or strictly control model-generated HTML before rendering.
- Avoid inserting chatbot responses with raw `innerHTML`.
- Add authentication if handling private medical documents.
- Add logging that avoids storing patient-sensitive information.
- Use HTTPS in production.
- Add a clear clinical disclaimer on all diagnostic result pages.
- Consider containerization with Docker for reproducible deployment.
...
- The models were trained on limited academic/public datasets rather than diverse hospital-scale datasets.
